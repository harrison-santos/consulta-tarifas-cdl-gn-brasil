...
class SulgasSpiderSpider(scrapy.Spider):
    ...
    def envia_sulgas(self, response, segmento_):#
        ...
        if(segmento_ == "INDUSTRIAL"):
            ...
            vetor_parcelas.extend(response.xpath('/html/body/div[3]/article/div[2]/ table[5] / tbody / tr[position() >= 3] / td[3] / text()').extract())
            vetor_parcelas = sulgas.calcula_imposto_parcela(icms, self.pis, self.confins, vetor_parcelas)
            dados = sulgas.organiza_faixa_tarifas_parcelas(vetor_faixa, vetor_tarifas, vetor_parcelas)
            yield from envia_dados(dados, sulgas.nome, segmento_, "INTERRUPTIVEL", "POSSUI")
            #INDUSTRIAL INTERRUPTIVEL

            #COGERAÇÃO HORÁRIO PONTA
            vetor_faixa = response.xpath('/html/body/div[3]/article/div[2]/table[7] / tbody / tr[position() > 1] / td[1] / p / span/text()').extract()
            vetor_faixa.extend(response.xpath('/html/body/div[3]/article/div[2]/table[7] / tbody / tr[position() > 1] / td[1] / p / text()').extract())
            for i in range(0, len(vetor_faixa)):
                vetor_faixa[i] = vetor_faixa[i].replace('\xa0', ' ')
            ...
            vetor_faixa[tam - 1] = vetor_faixa[tam - 1].replace('Acima de ', '').replace('acima de ', '')
            vetor_faixa[tam - 1] = vetor_faixa[tam - 1] + ' a 999.999.999'
            vetor_tarifas = response.xpath('/html/body/div[3]/article/div[2]/table[7] / tbody / tr[position() > 1] / td[3] / p / text()').extract()
            vetor_tarifas = sulgas.calcula_imposto_tarifa(icms, self.pis, self.confins, vetor_tarifas)
            vetor_parcelas = response.xpath('/html/body/div[3]/article/div[2]/table[7] / tbody / tr[position() > 1] / td[2] / p/text()').extract()
            vetor_parcelas[2] = vetor_parcelas[2].replace(',00', '')
            vetor_parcelas = sulgas.calcula_imposto_parcela(icms, self.pis, self.confins, vetor_parcelas)
            dados = sulgas.organiza_faixa_tarifas_parcelas(vetor_faixa, vetor_tarifas, vetor_parcelas)
            yield from envia_dados(dados, sulgas.nome, "COGERACAO", "HORÁRIO PONTA", "POSSUI")
            #COGERAÇÃO HORÁRIO PONTA

            #INDUSTRIAL COMPRIMIDO
            vetor_faixa = faixa_auxiliar
            consulta = response.xpath('/html/body/div[3]/article/div[2]/table[9]/tbody/tr[2]/td[2]/p/text()').extract()#posicao1 tarifa, posicao2 mmbtu
            vetor_tarifas = sulgas.calcula_imposto_tarifa(icms, self.pis, self.confins, [consulta[0]])
            parcelas = response.xpath('/html/body/div[3]/article/div[2]/table[9]/tbody/tr[2]/td[3]/p/text()').extract()
            vetor_parcelas = sulgas.calcula_imposto_parcela(icms, self.pis, self.confins, parcelas)
            dados = sulgas.organiza_faixa_tarifas_parcelas(vetor_faixa, vetor_tarifas, vetor_parcelas)
            ...
            vetor_faixa[tam - 1] = vetor_faixa[tam - 1].replace('Acima de ', '').replace('acima de ', '')
            vetor_faixa[tam - 1] = vetor_faixa[tam - 1] + ' a 999.999.999'
            vetor_tarifas = response.xpath('/html/body/div[3]/article/div[2]/table[1] / tbody / tr[position() > 1] / td[3] / text()').extract()
            vetor_tarifas = sulgas.calcula_s_imposto_tarifa(icms, self.pis, self.confins, vetor_tarifas)
            vetor_parcelas = response.xpath('/html/body/div[3]/article/div[2]/table[1] / tbody / tr[position() > 1 and position() <= 3] / td[2] / p / text()').extract()
            vetor_parcelas.extend(response.xpath('/html/body/div[3]/article/div[2]/table[1] / tbody / tr[position() >= 4 and position() <= 6] / td[2] / text()').extract())
            vetor_parcelas.extend(response.xpath('/html/body/div[3]/article/div[2]/table[1] / tbody / tr[position() >= 7] / td[2] / p / text()').extract())
            ...
            vetor_tarifas = [vetor_auxiliar[0].replace(' ', '')]
            vetor_tarifas = sulgas.calcula_imposto_tarifa(icms, self.pis, self.confins, vetor_tarifas)
